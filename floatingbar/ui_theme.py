...
def bind_picker_navigation(
    ...
    *,
    ...
    def focus_edge(index: int):
        rows[max(0, min(len(rows) - 1, index))].focus_set()

    window.bind("<Up>", lambda _event: (move_focus(-1), "break")[1], add="+")
    window.bind("<Down>", lambda _event: (move_focus(1), "break")[1], add="+")
    window.bind("<Home>", lambda _event: (focus_edge(0), "break")[1], add="+")
    window.bind("<End>", lambda _event: (focus_edge(-1), "break")[1], add="+")
    ...
    for row in rows:
        ...
        row.bind("<Home>", lambda _event: (focus_edge(0), "break")[1], add="+")
        row.bind("<End>", lambda _event: (focus_edge(-1), "break")[1], add="+")
    rows[0].focus_set()
